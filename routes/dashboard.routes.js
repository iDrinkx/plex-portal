...
router.get('/changelog', (_, res) => {
  try {
    ...
    // Convertir Markdown en HTML simplement (garder la mise en forme originale avec <pre>)
    let html = changelogContent
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;')
      .replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" target="_blank">$1</a>');

    // Wrapper dans un pre formaté pour préserver la mise en forme Markdown
    html = `<pre style="white-space: pre-wrap; word-wrap: break-word; font-size: 0.95em; line-height: 1.6; background: rgba(15,23,42,0.5); padding: 1.5rem; border-radius: 8px; border-left: 4px solid #34d399;">${html}</pre>`;
...
<head>
  ...
  <title>Changelog - Plex Portal v${version}</title>
  <style>
...
<body>
  <div class="container">
    <div class="header">
      ...
      <p style="margin-top: 1rem; color: #94a3b8;">Historique complet des versions et mises à jour</p>
    </div>
    <div class="content">
      ${html}
    </div>
    <div class="footer">
      <p>Mis à jour automatiquement lors de chaque release</p>
    </div>
  </div>
</body>
</html>`);
  } catch (err) {
    res.status(500).send('<h1>Erreur</h1><p>Impossible de charger le changelog</p>');
  }
});
